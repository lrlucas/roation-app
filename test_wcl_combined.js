...
async function test() {
  ...
  const query = `query { reportData { report(code: "${parse.report.code}") { masterData { abilities { gameID name icon } } events(fightIDs: [${parse.report.fightID}], dataType: Casts, startTime: ${parse.startTime}, endTime: ${parse.startTime + parse.duration}) { data } } } }`;
  ...
  if (gqlData.errors) console.log("GQL ERROR", JSON.stringify(gqlData.errors, null, 2));
  else {
     console.log('Abilities fetched:', gqlData.data.reportData.report.masterData.abilities.length);
     console.log('Events fetched:', gqlData.data.reportData.report.events.data.length);
  }
}
test();

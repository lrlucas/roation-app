...
async function test() {
  ...
  console.log('parse.startTime:', parse.startTime);
  
  const query = `query { reportData { report(code: "${parse.report.code}") { events(fightIDs: [${parse.report.fightID}], dataType: Casts, limit: 1) { data } } } }`;

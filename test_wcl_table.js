...
async function test() {
  ...
  const query = `query { reportData { report(code: "${parse.report.code}") { table(fightIDs: [${parse.report.fightID}], startTime: ${parse.startTime}, endTime: ${parse.startTime + parse.duration}, dataType: Casts) events(fightIDs: [${parse.report.fightID}], startTime: ${parse.startTime}, endTime: ${parse.startTime + parse.duration}, dataType: Casts, limit: 10) { data } } } }`;

...
async function test() {
  ...
  const query = `query { reportData { report(code: "${parse.report.code}") { table(fightIDs: [${parse.report.fightID}], dataType: Casts) events(fightIDs: [${parse.report.fightID}], dataType: Casts, limit: 10) { data } } } }`;
  ...
  console.log("With only fightIDs:", JSON.stringify(gqlData, null, 2));
}
test();

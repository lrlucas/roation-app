...
async function test() {
  ...
  const query = `query { reportData { report(code: "${parse.report.code}") { table(fightIDs: [${parse.report.fightID}], dataType: Casts) } } }`;
  ...
  console.log(JSON.stringify(gqlData.data.reportData.report.table.data.entries.slice(0, 5), null, 2));
}
test();
